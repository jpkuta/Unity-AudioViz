...
public class InstantiateCubes2 : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < cubeCount; i++)
        {
            ...
            {
                _sampleCube[i].transform.localScale = new Vector3(10, AudioPeer2._samplesLeft[i] * _maxScale + 2, 10);
            }
            if (_sampleCube != null && _useBuffer)
            {
                _sampleCube[i].transform.localScale = new Vector3(10, AudioPeer2._freqBand[_band] * _maxScale + 2, 10);
            }
        }
